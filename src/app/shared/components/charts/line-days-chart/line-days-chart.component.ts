...
@Component({
  ...
  templateUrl: './line-days-chart.component.html',
  styleUrl: './line-days-chart.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class LineDaysChartComponent {
  ...
  @tuiPure
  private generateRandomData({from, to}: TuiDayRange, initial: number): ReadonlyArray<[TuiDay, number]> {
    return new Array(TuiDay.lengthBetween(from, to) + 1)
        .fill(0)
        .reduce<ReadonlyArray<[TuiDay, number]>>(
            (array, _, i) => [...array, [from.append({day: i}),
                this.isE2E
                    ? initial
                    : Math.max((i ? (array[i - 1]?.[1] ?? 0) : initial) +
                        Math.random() * 10 -
                        5,
                        0,
                    ),
